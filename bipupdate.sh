#!/bin/bash
### BEGIN INIT INFO
# Provides:          Blackip Update
# Required-Start:    $remote_fs $syslog
# Required-Stop:     $remote_fs $syslog
# Default-Start:     2 3 4 5
# Default-Stop:      0 1 6
# Description:       capture blackips
...
echo "Checking Sum..."
a=$(md5sum $bip/blackip.tar.gz | awk '{print $1}')
b=$(cat $bip/blackip.md5 | awk '{print $1}')
	if [ "$a" = "$b" ]
	then 
		echo "Sum Matches"
		cd $bip
...
downloadbl 'https://www.malwaredomainlist.com/hostslist/ip.txt' && sleep 1
...
# CLEAN BLACKIP
awk -F'[.]' '{w=$1+0; x=$2+0; y=$3+0; z=$4+0; print w"."x"."y"."z}' blackip.txt > capture.txt
sort -u capture.txt | sort -t . -k 1,1n -k 2,2n -k 3,3n -k 4,4n > blackip.txt
echo "OK"
...
echo "Blackip for Ipset $date" >> /var/log/syslog
